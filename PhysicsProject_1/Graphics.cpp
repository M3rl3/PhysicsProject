...
void Graphics::Update() {
	...
	if (gdp::GDP_IsKeyPressed('f')) {
		...
		tank_ammo.part.velocity = Vector3(-36.f, 2.f, -36.f);
		...
		//enemy_tank.position = enemy_tank.GetPosition();

		while (tank_ammo.part.position.y >= 0) {
			
			/*tank_ammo.game_object->Position.x = tank_ammo.part.position.x;
			tank_ammo.game_object->Position.y = tank_ammo.part.position.y;
			tank_ammo.game_object->Position.z = tank_ammo.part.position.z;*/
...
void Graphics::CheckDist(Tank enemy, Particle p) {
	...
	std::cout << "\nBullet Landed at: " << p.position << std::endl;
	std::cout << "\nEnemy Position: " << enemy_tank.position << std::endl;
	
	float dist_tar = 0.f;
	if (enemy.position.z > p.position.z || enemy.position.x > p.position.x) {
